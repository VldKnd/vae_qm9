...
optimizer = optim.Adam(
    [
        #GRU Weights
        model.decoder.W_z.weight,
        model.decoder.U_r.weight,
        model.decoder.W_r.weight,
        model.decoder.W_h.weight,

        #Feature Aggregate Weights
        model.decoder.W.weight,
        model.decoder.U.weight,

        #Output Weights
        model.decoder.W_o.weight,
        model.decoder.U_s.weight,
    ], lr=lr)
...
verbose_df = pd.DataFrame(columns=["Step", "Loss", "KL", "Word", "Topo", "Assm", "Steo", "PNorm", "GNorm", "LR"])
...
for epoch in range(MAX_EPOCH):
    ...
    for it, batch in enumerate(dataloader):
        ...
        try:
            model.zero_grad()
            wloss, kl_div, wacc, tacc, sacc, dacc = model(batch, beta=0)
            wloss.backward()
            optimizer.step()
